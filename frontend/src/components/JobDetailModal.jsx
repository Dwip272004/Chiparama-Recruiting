...
import {
  ...
  AlertTriangle, CheckCircle, Tag, Briefcase, ChevronRight, Pencil
} from "lucide-react";
...
export function JobDetailModal({ job: initialJob, onClose, onUpdate }) {
                  ...
                  <section>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {job.top_skills.map((skill, i) => (
                        <span key={i} className="text-xs bg-indigo-50 text-indigo-700 border border-indigo-100
              ...
              <div className="p-6 bg-gray-50/60 space-y-5">
                ...
                {(job.location_city || job.location_full_address) && (
                  <DetailRow icon={MapPin} label="Location">
                    ...
                    {job.location_full_address && (
                      <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">{job.location_full_address}</p>
                    )}
                  </DetailRow>
                )}
                ...
                {(job.start_date || job.end_date) && (
                  <DetailRow icon={Calendar} label="Duration">
                    {job.start_date && <p><span className="font-medium">Start:</span> {fmtDateLong(job.start_date)}</p>}
                    {job.end_date   && <p><span className="font-medium">End:</span> {fmtDateLong(job.end_date)}</p>}
                  </DetailRow>
                )}
